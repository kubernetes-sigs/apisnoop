...
** Function for loading in the open api spec 
   ...
   #+BEGIN_SRC python :results output
     ...
     if release is not None:
       ...
       open_api = json.loads(urlopen(release_url).read().decode('utf-8')) # may change this to ascii
     ...
     # sql = Template("""
     #               WITH jsonb_array AS (
     #               SELECT jsonb_array_elements('${open_api}'::jsonb) as api_data)
     #               INSERT INTO open_api(release, release_date)
     #                  SELECT
     #                  (api_data->>'testname') as release,
     #                  (test_data->>'codename') as release_date,
     #                  (test_data->>'release') as release,
     #                  (test_data->>'description') as description,
     #                  (test_data->>'file') as file
     #                  from jsonb_array;
     #               """).substitute(open_api = open_api.replace("'","''"),
     #                               release_date = release_date)
     # plpy.execute((sql))
     # $$ LANGUAGE plpython3u ;
     # reset role;
