    ...
    #+BEGIN_SRC js
      ...
      const SunburstChart = (props) => {
        ...
        function handleMouseOut (node, event) {
          doUpdateQuery(zoomed: query.zoomed
...
** NEXT add Zoom function to the sunburst.
...
** TODO pass along a query when you click, that includes zoom depth and an array of the current node path.
