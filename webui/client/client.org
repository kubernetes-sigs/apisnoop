...
*** Routing Actions
    ...
    :END:
    We are using the feather-route-matcher made by henrik joreteg for this.  So we are pulling in the function called 'createRouteMatcher' but renaming it as routeMatcher .  More info can be found on the feather-route-matcher github.
...
*** Routing Reducer
    ...
    The goal is to listen to the location change and adjust the state accordingly.  We can then have our component always be showing the release related to the lastest route.
     ...
     #+BEGIN_SRC js
       ...
       var initialUrl = typeof window.location.pathname !== 'undefined' ? window.location.pathname : '/',

       const initialState = {
         url: typeof window.location.pathname !== 'undefined' ? window.location.pathname : '/',
         params: {},
         page: MainPage
       }
       ...
       // release: match.params.release ? path.params.release : 'master',
