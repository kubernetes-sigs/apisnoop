...
* Pod is smallest resource grouping in K8
  - Should be a complimentary services like my app, promethius exporter, envoy, data .... all things needed to run that one thing
...
* Deployments tied together is a service
  - Service is abstraction that will function as a load balancer for the pods. I give a deployment a name and an ip and that becomes the service.
