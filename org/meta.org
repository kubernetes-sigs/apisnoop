...
- [ ] Test that audit events loaded.
  ...
  The following query should return a count of =305025=
  ...
  #+begin_src sql-mode
  ERROR:  relation "audit_event" does not exist
  LINE 1: select count(distinct audit_id) from audit_event;
                                               ^
  #+end_src
...
#+BEGIN_SRC sql-mode
  ...
  select
   ...
   group by podspec_field;
#+END_SRC
** 304: PodSpec Field Review View
  :PROPERTIES:
  :header-args:sql-mode+: :tangle ../apps/hasura/migrations/304_view_podspec_field_report.up.sql
  :END:
...
#+NAME: hasura docker-compose
#+BEGIN_SRC yaml :tangle ../apps/docker-compose.yaml
...
** August
*** 23
    We went through a slew of debugging and restarts today, ending with us having a new docker container that holds everything needed to start api rainbow...but still not working successfully yet.  The next ask was to create a port based on a  user id and then have tmate access the container through this port via emacs.  Research on how to do this, and what it exactly means, eluded me, and so I thought the next best thing was to recap what we are trying to do to make sure I understand it.

    So far, we've been running api rainbow from the same sharing.io machine.  This machine has postgres running on it, with different users and db already set up.  We had containerized our hasura front end so that you could continually bring down and back up the graphql backend.  However, this required creating a folder named as $USER + '-hasura', and to create a symlink from our standard hasura folder.

    These things make it hard for people outside of ii to be able to run apisnoop rainbow, which we want them to do.  It also makes our whole setup fragile--since it requires sharing.io to always be running in pristine condition.  If osmething happened to the postgres instance, it'd be hard to replicate our work.

and so, we will make a docker container for _everything_.  Now, the only thing that's required for anyone to work on apisnoop rainbow is the git repo and docker, and they'd run a =docker-compose up= and have their own backend populated with raw_swaggers and audit_events. 

To be able to do this, though, we'll need to check and adjust some of the assumptions built into the code now:
- that the person will have access to sharing.io, as it's set as a default endpoint.
- that the person will know how to enter into postgres to run our go command
- That the person will know the location of our sources, in case they want to update which audit events and raw swaggers are loaded.
- That the person will understand postgres and how to start up a server and db once postgres is started.

  I think The ideal container will have it so someone can enter in a bucket and job in a sources.yaml in the root of apisnoop.  Then, they can run =docker-compose up= and it will start up everything needed at localhost:XXXX (with the port number being able to be passed as a flag).  This would start up postgres, pgadmin, and hasura.  The audit_events for the given bucket/job would load, as would the swaggers based on the version given in the metadata of the bucket.  All our views would load and be properly indexed.  The assigning of this port to some public url (like we do with sharing.io) should not be a part of the docker, as I imagine a decent amount of users would be running it local only.   However, the assigning of a port for the container, so someone can easily pop a shell into it, would be built in.  I think we'd also want the org-file integration tightly built, as it's a good thing to encourage.

I think I end up being confused on this as we are combining proxy management of sharing.io with docker with emacs and i'm not sure where one starts and stops.  Or, I'm not sure how much of what I enjoy about what we're doing (working within an org file and doublechecking with a url) is due to set ups in our sharing.io box that'd be hard to replicate.  I am keen to reduce the docker file further so there are no traces of local environments, and see how to host that on sharing.io without being dependent on any program in sharing.io 
...
*** .env
#+BEGIN_SRC shell :tangle ~/ii/apisnoop_v3/apps/.env
  PGADMIN_PORT=9001
  HASURA_PORT=9000
...
*** Restart Hasura
    ...
    #+BEGIN_SRC tmate
      ...
      
      # cd ~/ii/apisnoop_v3/
      #echo create a $USER-apps dir and link back to ../apps/*
      #mkdir -p $USER-apps && cd $USER-apps && ln -sf ../apps/* .
      #ls ../apps/.*env | xargs -n 1 -I X ln -sf X .
      cd ~/ii/apisnoop_v3/apps
      echo docker down, drop sql, rm migrations
      . .loadenv
      cp pgpass ~/.pgpass
      # cncf/apisnoopregexp rmatch CONN string
      ...
      export SOCKETFILE=$(ls /tmp/emacs$(id -u)/server* | head -1)
      # cd ~/ii/apisnoop_v3/$USER-apps \
      cd ~/ii/apisnoop_v3/apps \
       && docker-compose down -t 0 \
       && docker-compose rm \
       && docker volume rm -f $USER-hasura_migrations \
       && rm -f hasura/migrations/* \
       && emacsclient -s $SOCKETFILE -e "(org-babel-tangle-file \"~/ii/apisnoop_v3/org/meta.org\")" \
       && docker-compose build \
       && docker-compose up #\
       #&& echo "select * from load_audit_events('ci-kubernetes-e2e-gci-gce','1134962072287711234');" \
       #| psql \
       #&& echo "select * from audit_event_op_update();" \
       #| psql
       # && sleep 2 \
       ...
       #&& make rmatch \
       #&&  ./rmatch
       # psql -h 127.0.0.1 -p $(id -u)1 --user apisnoop
       # \
...
*** setting up the hasura postgresql-permissions
 ...
 #+BEGIN_SRC sql-mode :noweb yes :notangle ../apps/hasura/db_setup.sql
...
** Local Variables
   - Note taken on [2019-08-23 Fri 17:06]
** connect to db
...
# eval: (server-start)
# eval: (concat server-socket-dir "/" server-name)
# End:
