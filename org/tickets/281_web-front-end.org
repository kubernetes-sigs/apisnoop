...
** TODO [1/5] Add endpoints filter that reduces number of endpoints in sunburst
   regex filter for:
   - useragent
   - testtag
   - test
...
*** TODO [3/6] create filteredEndpoints store.
...
**** DONE filter by is_untested
     CLOSED: [2020-01-20 Mon 02:23]
**** TODO filter by whether it is hit by a useragent matching regex.
**** TODO filter on whether it's hit by test matching regex
**** TODO filter on wheter its hit by test with test_tag that matches regex
*** TODO endopoints that make up sunburst based on filteredEndpoints
*** TODO coverage stats based on filtered endpoints
