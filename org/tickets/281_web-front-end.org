...
** 
** IN-PROGRESS [0/8] fetch endpoint data from url instead of hard coded
*** IN-PROGRESS On page load, fetch metadata from bucket_job_swagger and add to store
    this lets us know all the available buckets and jobs.
*** TODO Derive Latest job for each bucket
*** TODO Derive default bucket/job.
...
*** TODO Switch index page to just say apisnoop
*** TODO When you go to coverage, display default bucket/job in sunburst
*** TODO When you go to coverage/:bucket display latest job for this bucket
*** TODO If bucket in url not available in bucket/jobs....give message and display latest bucket job.
*** TODO When you go to coverage/[bucket]/[job] display sunburst for this bucket/job.
    If the job does not match one found in this bucket, then display default bucket/job.
