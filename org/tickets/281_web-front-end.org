...
** TODO [0/4] display test information to right of sun burst
*** TODO Write a tests view with bucket, job, name, test tags, operation_id, timestamp?, conformance
...
*** TODO Write a hasura array relationship so each endpoint has a tests section that shows all tests that are hit by it.
*** TODO Write a test_tags view with bucket, job, name, operation_id.
*** TODO Write a hasura query for all the test tags of an endpoint.
