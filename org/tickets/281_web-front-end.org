...
** TODO add trac,ing to endpoint_coverage in our hasura migrations
...
*** DONE if :bucket/:job both unavailable display default bucketJob
    CLOSED: [2020-01-08 Wed 22:10]
** IN-PROGRESS [0/5] Root of sunburst based on url params
*** IN-PROGRESS Add level variable to Sunburst component, that determines root of sunburst.
*** TODO Add Category variable to sunburst, that, with level, determines root of sunburst.
*** TODO given :bucket/:job/:level, if level is valid, pass it along to Sunburst component.
*** TODO given :bucket/:job/:level/:category, if level and category are valid, pass them along to Sunburst component.
*** TODO give notice if either level or category are present but invalid.
** TODO Highlighted Endpoint based on url params
*** If endpoint variable present, show sunburst with root set to endpoints level and category, and only endpoint path lit up in sunburst.
*** Put in checks for invalid level, category, and/or endpoint.
** TODO [/] click to set sunburst in place at current node.
*** TODO onClick, set url based on current node and its parents. 
...
** TODO ensure we can get newest test data from graphl (latest test run works in loading app from new clusteR)
* Footnotes   
:PROPERTIES: 
:CUSTOM_ID: footnotes 
:END: 
...
*** Verify Pods Running
    ...
    apisnoop-auditlogger-5f6c4cb8c5-ljkss   1/1     Running   0          5m17s
    hasura-777765b7d4-spkrt                 1/1     Running   0          5m17s
    postgres-b59f6c9c4-mpdrv                1/1     Running   0          5m17s
    #+end_src
