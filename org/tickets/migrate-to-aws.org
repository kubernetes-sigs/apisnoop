...
** workflows
  ...
  apisnoop:deployment/hasura    hasura              raiinbow/hasura:v2020.04.09-1    ready    locked
  ...
  apisnoop:deployment/postgres  postgres            raiinbow/postgres:v2020.04.09-1  ready    locked
  apisnoop:deployment/webapp    webapp              raiinbow/webapp:v2020.04.09-1    ready    locked
  #+end_example

** images
  #+begin_src shell
    fluxctl list-images -n apisnoop -l 3 2>/dev/null
  #+end_src
  ...
  apisnoop:deployment/hasura    hasura              raiinbow/hasura          
                                                    ...
                                                    |   v2020.04.08-3        07 Apr 20 21:47 UTC
                                                    '-> v2020.04.09-1        07 Apr 20 21:47 UTC
  apisnoop:deployment/pgadmin   pgadmin             dpage/pgadmin4           
                                                    |   snapshot             07 Apr 20 00:01 UTC
                                                    |   2020-03-30-1         30 Mar 20 12:25 UTC
                                                    |   4                    30 Mar 20 12:25 UTC
                                                    : (9 image(s) omitted)   
  ...
  apisnoop:deployment/postgres  postgres            raiinbow/postgres        
                                                    ...
                                                    |   v2020.04.08-3        07 Apr 20 21:51 UTC
                                                    '-> v2020.04.09-1        07 Apr 20 21:51 UTC
  apisnoop:deployment/webapp    webapp              raiinbow/webapp          
                                                    '-> v2020.04.09-1        08 Apr 20 15:49 UTC
                                                        v2020.04.08-2        07 Apr 20 22:33 UTC
                                                        v2020.04.08-3        07 Apr 20 22:33 UTC
  #+end_example
...
* update data
  :PROPERTIES:
  :header-args:tmate+: :window psql
  :END:
  #+begin_src tmate
     POSTGRES_POD=$(kubectl get -n apisnoop pods | grep postgres | awk '{print $1}')
     kubectl exec -n apisnoop $POSTGRES_POD -ti -- psql
  #+end_src
  #+begin_src tmate
  ...
  #+begin_src tmate :window psql
    ...
    # This is current master, we'll get the latest when it's loaded
    # 1.18
    select * from load_swagger('ci-kubernetes-e2e-gce-cos-k8sbeta-default');
    # -- , '1244368729727307776', NULL);
    select * from load_audit_events('ci-kubernetes-e2e-gce-cos-k8sbeta-default');
    # -- ,'1244368729727307776');
    update bucket_job_swagger SET job_timestamp='2020-03-24 00:00:00'::timestamp WHERE bucket = 'ci-kubernetes-e2e-gce-cos-k8sbeta-default';
    # 1.17
    select * from load_swagger('ci-kubernetes-e2e-gce-cos-k8sstable1-default'); --, 'TAGME', NULL);
    select * from load_audit_events('ci-kubernetes-e2e-gce-cos-k8sstable1-default'); -- ,'TAGME');
    UPDATE bucket_job_swagger SET job_timestamp='2019-12-09 00:00:00'::timestamp WHERE bucket = 'ci-kubernetes-e2e-gce-cos-k8sstable1-default';
    # 1.16
    select * from load_swagger('ci-kubernetes-e2e-gce-cos-k8sstable2-default'); --, 'TAGME', NULL);
    select * from load_audit_events('ci-kubernetes-e2e-gce-cos-k8sstable2-default'); --, 'TAGME');
    UPDATE bucket_job_swagger SET job_timestamp='2019-10-22 00:00:00'::timestamp WHERE bucket = 'ci-kubernetes-e2e-gce-cos-k8sstable2-default';
    # 1.15
    select * from load_swagger('ci-kubernetes-e2e-gce-cos-k8sstable3-default'); --, 'TAGME', NULL);
    select * from load_audit_events('ci-kubernetes-e2e-gce-cos-k8sstable3-default'); --, 'TAGME');
    UPDATE bucket_job_swagger SET job_timestamp='2019-06-20 00:00:00'::timestamp WHERE bucket = 'ci-kubernetes-e2e-gce-cos-k8sstable3-default';
    # 1.14
    select * from load_swagger('ci-kubernetes-e2e-gce-cos-k8sstable3-default', '122841422048421888', NULL);
    select * from load_audit_events('ci-kubernetes-e2e-gce-cos-k8sstable3-default', '122841422048421888');
    UPDATE bucket_job_swagger SET job_timestamp='2019-03-24 00:00:00'::timestamp WHERE bucket = 'ci-kubernetes-e2e-gce-cos-k8sstable3-default' and job = '1216086104600481804';
    # refresh views
...
** image building
   #+begin_src tmate :window image
     for APP in auditlogger webapp postgres hasura ; do
         ...
         export IMAGE=raiinbow/$APP:v2020.04.09-1
