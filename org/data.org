...
CREATE OR REPLACE VIEW "public"."events" AS 
 SELECT (audit_events.event -> 'auditID'::text) AS uuid,
    (audit_events.event -> 'level'::text) AS level,
    (audit_events.event -> 'verb'::text) AS verb,
    (audit_events.event -> 'requestURI'::text) AS uri,
    (audit_events.event -> 'userAgent'::text) AS useragent,
