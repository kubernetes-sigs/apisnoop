...
** Imports
   ...
   #+end_src
** Constants
   #+NAME: global func constants
   #+begin_src python
   GD['GCS_LOGS'] = "https://storage.googleapis.com/kubernetes-jenkins/logs/"
   GD['DEFAULT_BUCKET'] = "ci-kubernetes-gci-gce"
   #+end_src
** define get_json
   #+NAME: define get_json
   #+begin_src python
     def get_json(url):
...
** put it all together
   #+NAME: determine_bucket_and_job
   #+begin_src sql-mode
     set role dba;
     DROP FUNCTION IF EXISTS set_global_functions;
     CREATE OR REPLACE FUNCTION set_global_functions()
       RETURNS void AS $$
       <<global_funcs imports>>

       <<define get_json>>

       <<global_func constants>>
       GD['get_json'] = get_json
       $$ LANGUAGE plpython3u ;
       reset role;
   #+end_src
...
** 102: Refined Load Swagger
  :PROPERTIES:
  :header-args:sql-mode+: :tangle ../apps/hasura/migrations/102_function_load_swagger.up.sql
  :END:
    #+NAME: zload_swagger.sql
    #+BEGIN_SRC sql-mode :noweb yes :results silent
      set role dba;
      DROP FUNCTION IF EXISTS zload_swagger;
      CREATE OR REPLACE FUNCTION zload_swagger(
        custom_bucket text default null,
        custom_job text default null,
        live boolean default false)
      RETURNS text AS $$
...
**** Our Imports and Constants
     We have a small set of dependencies, the request library for fetching and parsing urls, os for reading environment variables passed on in our docker ubild, json for reading our fetched data, and some string templating capabilities for when we write out a sql function inside of python (yes we are doing sql inside python inside sql). 
     #+NAME: load_swagger: imports
     #+begin_src python
       from string import Template

       def testing():
           GCS_LOGS="https://storage.googleapis.com/kubernetes-jenkins/logs/"
           DEFAULT_BUCKET="ci-kubernetes-e2e-gci-gce"
           get_json = GD['get_json']
           baseline_bucket = os.environ['APISNOOP_BASELINE_BUCKET'] if 'APISNOOP_BASELINE_BUCKET' in os.environ.keys() else DEFAULT_BUCKET 
           bucket =  baseline_bucket if custom_bucket is None else custom_bucket
     ...
     For constants, we want to define the source url to grab job metadata from, and the default bucket we will look at in case no bucket is passed in the function's arguments or APISnoop's env vars.

     #+NAME: load_swagger: constants
     #+begin_src python
       GCS_LOGS="https://storage.googleapis.com/kubernetes-jenkins/logs/"
       DEFAULT_BUCKET="ci-kubernetes-e2e-gci-gce"
     #+end_src
    
**** Establish our bucket and job.
    
    #+NAME: scratch_load_swagger.py
    #+BEGIN_SRC python :eval never :exports code
...
**** Bring it all together
     #+NAME: load_swagger2.py
     #+begin_src python
     <<load_swagger: imports>>
     #+end_src
...
** 999: replace metadata
   :PROPERTIES:
   :header-args:yaml+: :tangle ../apps/hasura/migrations/999_replace_metadata.up.yaml
   :header-args:yaml+: :comments org
   :END:
