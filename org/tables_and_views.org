...
** understanding python
    #+NAME: determine_bucket_job.sql
    #+BEGIN_SRC sql-mode :noweb yes :results silent
      set role dba;
      DROP FUNCTION IF EXISTS testing_python;
      CREATE OR REPLACE FUNCTION testing_python()
      RETURNS text AS $$
      from snoopUtils import determine_bucket_job
      return determine_bucket_job()
      $$ LANGUAGE plpython3u ;
      reset role;
    #+END_SRC
    
    #+begin_src sql-mode
    select * from testing_python();
    #+end_src
    
    ah huzzah!
    
    This is a test for bringing in our own python modules into the plpython system.  We have a couple of functions that are used in multiple spots in our codebase.  We also work with some fairly complex functions that can be hard to debug when they're being run inside sql.  I had the idea of bringing them in as a module, so that we could write and test the python separate from the sql.

   I found where the python packages are installed in the postgres server (~/usr/lib/local/python3.7/dist-packages~), so I wrote up a simple one called ~snoopUtils~ which had one of our self-contained functions.  I then appended the postgres Dockerfile to copy this snoopUtils file over.   Then I tested it with the above function.  It works without a lot of fuss!  I think this is a good path to go down, though am keen to hear from more pythonic minds. 
