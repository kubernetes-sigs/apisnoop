...
*** Create Table
 ...
 #+BEGIN_SRC sql-mode :results silent
   select * from print_migration('100: bucket_job_swagger');
...
#+BEGIN_SRC sql-mode
   select * from print_migration('110: raw_audit_event');
...
#+BEGIN_SRC sql-mode :noweb yes
  select * from print_migration('111: load_audit_event_function');
  set role dba;
...
#+BEGIN_SRC sql-mode 
  select * from print_migration('200: api_operation_material');
...
#+BEGIN_SRC sql-mode
  select * from print_migration('220: api_operation_parameter_material');
...
*** Create
    ...
    #+BEGIN_SRC sql-mode
    select * from print_migration('300: audi_event');
...
** 500: Endpoint Coverage View
   ...
   #+BEGIN_SRC sql-mode
     select * from print_migration('500: endpoint_coverage');
...
** 520: stable endpoint_stats_view
   ...
   #+BEGIN_SRC sql-mode
     select * from print_migration('520: stable_endpoint_stats');
...
** 530: Change in Coverage 
   ...
   #+BEGIN_SRC sql-mode :results replace 
     select * from print_migration('530: change_in_coverage');
...
** 540: Change in Tests 
   ...
   #+begin_src sql-mode
     select * from print_migration('540: change_in_tests');
...
** 910: Populate Swaggers Up
  ...
  :END:
  #+begin_src sql-mode
    select * from print_migration('910: populate swaggers');
...
** 920: Populate Audits Up
  ...
  :END:
  #+begin_src sql-mode
    select * from print_migration('920: populate audit_events');
...
* 000 Meta Function                                                 :ARCHIVE:
   We want to know when each migration runs and see it in our logs.  So we'll create a function that outputs the current migration.
   We have our org mode setup with each heading named after the migration, and so to be lazy and repeatable, we'll make an org-mode function that gets the heading, and pass this in as the argument to our print_migration function
** get_heading_function
   #+NAME: get_heading
   #+begin_src elisp 
...
** 010: Print Header to Console
  :PROPERTIES:
  :header-args:sql-mode+: :tangle ../apps/hasura/migrations/010_print_migration_function.up.sql
  :END:
  #+begin_src sql-mode :noweb yes
  CREATE OR REPLACE FUNCTION print_migration(migration varchar)
    RETURNS void AS
  $$
    BEGIN
        RAISE NOTICE 'Beginning Migration: %', migration;
    END;
  $$
  ...
  #+begin_src sql-mode :noweb yes
  select * from print_migration('10: good stuff');
  #+end_src

  #+RESULTS:
  #+begin_src sql-mode
  NOTICE:  Beginning Migration: 10: good stuff
   print_migration 
  -----------------
