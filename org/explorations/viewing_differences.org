...
** TODO List Endpoints covered in new not covered in old
   This is basically the same as our tests, this time showing the distinct endpoints.  
   If we are only concerned with stable endpints, then we want to make sure that alpha and beta are removed from the query.
       ...
       old_run as (
         ...
         (
           SELECT DISTINCT
            ...
            WHERE ae.useragent LIKE 'e2e.test%' and useragent LIKE '%--%'
   ...
   Zero, which makes sense if our coverage went down.  If the next task's query returns 0, then something is wrong.
    
** TODO list endpoints covered in old job but not new
   This should return a number of endpoints.  if it's 0, the query needs adjusting.
   ...
   One thing that'd be good to know is if any of these were conformance tested, though.  These are just showing tested in general.
