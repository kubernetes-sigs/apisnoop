...
***** Notes
    We now have each audit.log path at /api/v1/audits:auditLogLink...which reminds me that this should be changed.
****** Older Notes
...
***** Grabbing the link for the audit-lob, using web parsing
      I think I could just do a simple request to our gcs-site, navigating to the specific build number's page, and then use web-scraping to find the a href for kube-audit.
      I'll make a hook for when a new audit is added, that pings the link provided for that audit.
