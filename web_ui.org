...
*** CommitCard
    ...
    :END:
   Component for displaying the conformance-gce commits
    #+NAME: Commit Card
    #+BEGIN_SRC js
      import React from 'react'
...
*** DONE Take every conformance-gce dashboard_tab.name, and list them.
    CLOSED: [2018-10-04 Thu 17:39]
...
*** DONE Take every conformance-gce dashboard_tab.name, and separate them to dev and stable using regex on the name (dev) versus (release).
    CLOSED: [2018-10-04 Thu 17:40]

*** NEXT For each of these, display their latest build number.  (perhaps, if they aint' there, go back in time until you find something that works and have a note that says 'latest isn't stable!' or something.)
    This could be done by querying the correct gubernator thang as pulled from the test_group gcs-prefix.  But I could say that this is how I do the config.yaml stuff too...which makes me wonder whether feathers is needed at all.  I thought it was good for performance, or stability, but should check with a senior dev type to figure out the best process, once we got some working things in.
