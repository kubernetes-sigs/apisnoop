...
**** selectFilteredAndZoomedEndpoints
     *FilteredEndpoints  Zoom -> Endpoints*
     _Given a set of endpoints, and a zoom level and location, return endpoints whose location matches the zoom._
     Zoom is an object with {depth, level, category, endpoint} that is derived from a mouseclick on the sunburst.
     Depth is which ring (level, category,endpoint) was clicked.  The ...location is the node path leading to that depth (e.g. clicking on level only shows the exact level, clicking on an endpoint shows the endpoint, the category it is a part of, and the level it is in.)
