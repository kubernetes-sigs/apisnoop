...
** Search Container
   :PROPERTIES:
   :header-args: :tangle ./src/components/search-container.js
   :END:
...
*** selectActiveTests
    *◊Endpoint ◊Tests -> ◊ActiveTests*
    ...
    #+BEGIN_SRC js :tangle no
     ...
     #+END_SRC
*** selectActiveTest
    *◊Query, ◊ActiveTests -> ◊ActiveTests
    /Return the test, from activeTests, that matches the query./
...
** IN-PROGRESS Make Sure Summary has everything it needs
...
** TODO Fix activeStats so it shows the current Zoom if there is no Query.
