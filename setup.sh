...
export NS=ii
export KUBEMACS=gcr.io/apisnoop/kubemacs:0.9.24
...
docker pull $KUBEMACS # cache into docker socket
kind load docker-image --nodes kind-worker $KUBEMACS # docker->kind
...
PODNAME=$(kubectl get pod --selector=app=kubemacs -o name  | sed s:pod/::)
