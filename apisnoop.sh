...
DEFAULT_STAGING="./data-gen/data"
DEFAULT_DEST="./data-gen/outdata"
if [ $# -eq 0 ]; then
  print_help
elif [ $1 = "--all" ]; then
  ...
  ./data-gen/downloadArtifacts.py ${2:-$DEFAULT_SOURCE} ${3:-$DEFAULT_STAGING}
elif [ $1 = "--process" ]; then
  ./data-gen/processArtifacts.py ${2:-$DEFAULT_STAGING} ${2:-$DEFAULT_DEST} > ./data-gen/processArtifacts.sh
  bash ./data-gen/processArtifacts.sh
elif [ $1 = "--install" ]; then
  pip install -r ./data-gen/requirements.txt
else
